...
export const useChat = ({
  ...
  // Call OpenAI API
  const callOpenAI = async (userMessage: string, signal: AbortSignal): Promise<string> => {
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      // Call OpenAI API
      const assistantResponse = await callOpenAI(content, abortControllerRef.current.signal);
  ...
  }, [isLoading, addMessage, onMessageSent, onMessageReceived, onError, apiKey]);
